...
object SearchAssocition extends SearchAssocitionConf with DataRetrieval{
  ...
  def main(args: Array[String]) {
    ...
    try{
      ...
      val userPlayInfoInit = sparkSession.
        sql("SELECT transformSid(videosid) as sid, accountid as accountId, key_hour as time, userid as userId " +
      ...
      /**
        *  "helios_cms_mysql":{"driver":"com.mysql.jdbc.Driver",
        ...
        在此类中增加appVersionFlag=0，来保证只推低版本的数据；对于ota24版本，则不需要做任何限制
        * */
      ...
      /*val sql = "select sid, content_type from mtv_basecontent where id >= ? AND id <= ? " +
        "and type=1 and status=1 and video_type in (0,1) and appVersionFlag=0 and content_type in " + videoType*/

      val metaData = getMetaDataFromMysql(sc, "helios_cms_mysql", "mtv_basecontent", sql,
        r => (CodeIDOperator.codeToId(r.getString(1)).toInt, r.getString(2))).toDF("sidMysql", "contentType")
      ...
      val topVideoData = userPlayInfo.selectExpr("contentType", "sid", "1 as num").
        ...
        map(r => (r.getString(0), (r.getInt(1), r.getLong(2)))).
        groupByKey().map(r => {
      ...
      for(i <- 0 to 9)
        KafkaOps.writrString2Kafka(topVideoJsonData.toString(), heliosSearchTopVideoTopic)
      ...
      KafkaOps.writeRDD2Kafka(unionData, heliosSearchTopContentTypeTopic)
