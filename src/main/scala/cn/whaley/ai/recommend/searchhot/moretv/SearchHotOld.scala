...
object SearchHotOld extends SearchHotConf with DataRetrieval{
    ...
    try{
      ...
      val medusaUserShortPlayInfo = sparkSession.
        sql("SELECT searchtext as searchText, contenttype as contentType, " +
          ...
          s"WHERE contenttype in ${shortVideoType} " +
          "AND searchtext != '' " +
          "AND searchtext IS NOT NULL " +
          s"AND key_day > ${startDate}")

      // 合并两份日志信息得到RDD["searchText", "contentType", "videoSid", "num"]
      val resultData = medusaUserLongDetailInfo.union(medusaUserShortPlayInfo).
      ...
      // 处理数据
      DataProcess.sendData2Kafka(resultData, moretvSearchHotTopicOld, moretvMysqlName)
      //对应电视猫2.x版本,可直接忽略
      //resultData.show(10)
      ...
      case e: Exception =>
        val info = prefix + "[moretv][searchHotOld]" + "[" + HDFSOps.today + "]"
